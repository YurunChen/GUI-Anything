...
export function deriveActivityState(input: {
  ...
  return { label: 'Idle', tone: 'idle', spinning: false };
}
...
export function buildOutcomeSummary(input: {
  ...
  return `Outcome summaries:${summaryCount} | wiki saved:${savedCount} skipped:${skippedCount} failed:${failedCount} | errors:${errorCount}`;
}
