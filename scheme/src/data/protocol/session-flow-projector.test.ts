...
import { buildFlowGraphSnapshot } from './session-flow-projector';
...
describe('buildFlowGraphSnapshot', () => {
  it('builds deterministic nodes and edges', () => {
    ...
    expect(snapshot.edges.map((e) => e.relationship)).toEqual(['side', 'side']);
  ...
  it('uses fallback parent when declared parent is missing', () => {
    ...
    expect(snapshot.edges[0].relationship).toBe('side');
  });
  ...
  it('uses the question title for active exploration without flowchart hint', () => {
    const snapshot = buildFlowGraphSnapshot({
      sessionId: 's6',
      explorations: [
        makeExploration('exp_1', 100, 'running', 0, '分析下当前的项目'),
      ],
      summaries: {},
    });
    expect(snapshot.nodes).toHaveLength(1);
    expect(snapshot.nodes[0].label).toBe('分析下当前的项目');
  });

  it('keeps the question title after completion when hint is not ready', () => {
    const snapshot = buildFlowGraphSnapshot({
      sessionId: 's7',
      explorations: [
        makeExploration('exp_1', 100, 'complete', 0, '分析下当前的项目'),
      ],
      summaries: {},
    });
    expect(snapshot.nodes).toHaveLength(1);
  ...
  it('does not use markdown summary text as the fallback node title', () => {
    const snapshot = buildFlowGraphSnapshot({
      sessionId: 's10',
      explorations: [
        makeExploration('exp_1', 100, 'complete', 0, '分析下当前的项目'),
      ...
      summaries: {
        exp_1: '## GUI-Anything 项目分析 ### 📌 一句话定位 **GUI-Anything** 是 Claude Code 的侧观察器和记忆层',
      },
    });
  ...
  it('uses catalog intent_key for badge, not node_id slug', () => {
    const snapshot = buildFlowGraphSnapshot({
      sessionId: 's9',
      explorations: [makeExploration('exp_1', 100, 'complete')],
      summaries: { exp_1: 'trace wiki chain' },
      flowchartHints: {
        exp_1: hint('wiki_chain_review', '梳理 wiki 链路', null, 'trunk', 'high', 'explore'),
      },
    });

    expect(snapshot.nodes).toHaveLength(1);
    expect(snapshot.nodes[0].intentKey).toBe('explore');
  });
