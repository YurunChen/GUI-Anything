...
export function buildFlowGraphSnapshot(input: BuildFlowGraphInput): FlowGraphSnapshot {
  ...
  for (const bucket of bucketList) {
    ...
    edges.push({
      ...
      to: bucket.scopedId,
      relationship: mapBranchTypeToRelationship(bucket.branchType),
    });
  }
...
function buildRequirementTitle(question: string, summaryText: string): string {
  const candidate = question || summaryText;
  const normalized = cleanIntentTitle(candidate);
  ...
  return truncate(firstClause || '未命名需求', FALLBACK_TITLE_MAX_LEN);
}
...
function mapBranchTypeToRelationship(branchType: FlowchartHint['branchType']): FlowGraphEdge['relationship'] {
  if (branchType === 'parallel') return 'side';
  if (branchType === 'repair') return 'repair';
  if (branchType === 'merge') return 'merge';
  return 'main';
}

function normalizeNodeTitle(rawTitle: string, fallbackTitle: string): string {
  const cleaned = cleanIntentTitle(rawTitle)
    .replace(/^(围绕|针对|关于)\s*/u, '')
    .replace(/[。！？.!?]+$/u, '')
    .trim();
  const title = cleaned || cleanIntentTitle(fallbackTitle) || '未命名需求';
  return truncate(title, FALLBACK_TITLE_MAX_LEN);
}

function cleanIntentTitle(value: string): string {
  return value
    .replace(/[`*_~]/g, '')
    .replace(/#{1,6}\s*/g, '')
    .replace(/[“”"']/g, '')
