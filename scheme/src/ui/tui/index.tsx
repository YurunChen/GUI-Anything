...
import { startObserverStream } from '../../runtime/observer-stream';
...
export function ActivityPanel({ prompt, enableFlow }: ActivityPanelProps): ReactNode {
  ...
  useEffect(() => {
    ...
    const flowRecorder = enableFlow
      ? new FlowRecorder(new FlowStore(), `flow_${Date.now()}`, prompt, process.env.CLAUDE_MODEL)
      : undefined;
    ...
    startObserverStream({
      ...
      onEvent: (line) => {
        ...
        for (const event of events) {
          ...
          flowRecorder?.addEvent(event, line);
        }
      },
      onExit: () => {
        flowRecorder?.finalize();
        // Async: generate summary via Claude subagent
        if (flowRecorder) {
          const sessionId = flowRecorder.getSessionId();
          const sessionNodes = flowRecorder.getStore().getSession(sessionId).nodes;
          generateFlowSummaryAI(prompt, sessionNodes, process.env.CLAUDE_MODEL).then(summary => {
            flowRecorder!.getStore().insertSummary(sessionId, summary.title, summary.content, summary.nodeRefs);
          }).catch(() => {});
        }
        setRunning(false);
      }
