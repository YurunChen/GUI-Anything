...
| [release-checklist.md](release-checklist.md) | GitHub release 前检查 |
...
`ga flow` 进入 Zellij 双栏 TUI。项目演进 HTML 由 Observer 快捷键触发：`h` 导出并打开 deterministic HTML，`r` 用 AI enrichment 重生成；`ga flow --watch --open` 启动 live browser sidecar。
...
| `FLOW_NOTIFY_WECHAT_SERVICE_HOST` / `FLOW_NOTIFY_WECHAT_SERVICE_PORT` | 微信本地服务绑定地址与端口（默认 `127.0.0.1` / `8765`） |
...
Flow 内 HTML 入口：`h` 导出并打开 deterministic 项目演进 HTML，`r` 用 AI enrichment 重生成；CLI 入口 `ga flow --watch --open` 提供 live browser sidecar。
...
Personality runtime：`resolveLiveCodingPersona` 以三个 completed exploration 为稳定窗口刷新；`buildPersonalityStripInfo` 按 `FLOW_LOCALE` 选择中英文文案；`PersonalityStrip` 在命令栏上方展示 `「Personality」 · 名称 · intro/catchphrase/devStyle` 紧凑条。HTML evolution export 保留完整 persona 卡片。
